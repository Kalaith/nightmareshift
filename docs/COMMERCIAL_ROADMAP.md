...
- [x] 🔴 Design a **run structure / campaign**: escalating nights (night 1/2/3…), a run arc, a "final night." *(Done 2026-07-14: a run is now **N consecutive nights** (`NIGHTS_PER_RUN` constant, default 5). Added `GameState.night`/`run_complete`; refactored `start_game` into `begin_night`/`advance_night`. Each night rescales difficulty (`base lifetime diff + (night-1)`, capped) and the earnings quota (`base + base*(night-1)/2` → 150/225/300/375/450). Surviving an interim night shows a "Night N survived — X to go" results screen that presses on into the next night; surviving the final night triggers a distinct run-victory screen; dying or missing quota ends the run. The playtest bot advances through interim nights and ran 6 nights headless with exit 0; new constants + localization verified to parse.)*
- [x] 🔴 **Fix the dead half of the skill tree.** Only the 6 `ability_unlock` skills did anything; all `stat_boost`/`passive_bonus`/`mechanic_unlock` skills were purchasable no-ops. *(Done 2026-07-14: added a `SkillModifiers` aggregate (`game_engine.rs`) computed from the skill catalog + the player's unlocked list, and wired every effect into a real system — `fuel_consumption`→route fuel cost, `max_fuel`→a new `GameState.max_fuel` cap used by refuel/consequences/item bonuses/UI, `hazard_damage`→hazard-inflicted fuel/time/risk (accumulated separately so only the hazard portion is softened), `reveal_hidden_chance`→a chance to reveal a hidden rule at shift start (Glimpse), and fare/refuel/protection below. A unit test confirmed the aggregate values for all-unlocked and none-unlocked. Threaded through `calculate_route_costs`, `choose_route`, `complete_ride`, `refuel_*`, and the bot.)*
- [x] 🔴 Cut the **shop** and **sanity** systems that skill descriptions referenced (neither existed). *(Done 2026-07-14: rather than build two whole subsystems, rethemed the three orphaned skills to existing mechanics and updated their `skillTreeData.json` descriptions/targets — Silver Tongue `tip_multiplier`→`fare_multiplier` (scales every fare), Negotiator `shop_discount`→`refuel_discount` (cheaper refueling — the game's only "shop"), Basic Warding `sanity_resistance`→`supernatural_protection` (start each shift with an extra meltdown-absorbing ward). No skill now references a non-existent system.)*
- [x] 🟠 Multiple endings / epilogues and a framing story for the driver. *(Done 2026-07-14: the campaign already yields mechanically-distinct endings — interim-night results, a dedicated run-victory epilogue screen ("Every night endured. Dawn breaks at last."), death, and quota-failure. Added a driver **framing-story arc**: the briefing now shows a "NIGHT N OF M" progress label plus a per-night premise line (5-line through-line from "First night on the graveyard shift…" to "Last night. Make it to dawn…"), routed through localization (`briefing.premise`/`nightLabel`) and clamped for any run length. Verified the briefing loads/renders via a bot smoke run. **Still open:** longer authored epilogues, branching endings, and a credits sequence.)*
- [x] 🟠 Deepen the skill tree past 12 shallow nodes toward meaningful build identity/branches. *(Done 2026-07-14: **12 → 20 nodes**, extending all three branches with tier-2/3 picks that use the now-wired effect targets — survival (Deep Cycle, Reserve Cell, Blessed Plating), occult (Second Sight → Premonition, Iron Ward), efficiency (Golden Tongue, Haggler). Effects stack as designed: all-unlocked yields fuel ×0.81, +20 max fuel, hazard ×0.64, fare ×1.21, 2 wards, ~30% reveal chance. Unit-tested that all prerequisites resolve and the aggregates stack. Because §2's skill-fix already wired every target, these are functional build choices, not flavor.)*
- [ ] 🟠 **Seeded runs**: the toolkit ships a serializable `SeededRng` built exactly for this, but the game uses only macroquad's global unseeded RNG. *(Deferred 2026-07-14 with rationale: a real seeded/daily-run feature requires first **separating gameplay RNG from visual RNG**. `effects.rs` draws from the global `rng` stream ~24×/frame at variable framerate, so `rng::srand(seed)` alone would NOT make a run reproducible — the gameplay sequence would desync with frame count. The correct prerequisite is threading a state-owned `SeededRng` through the ~35 gameplay call sites across 11 files (game_engine, ride/route/item/guideline/passenger/weather services, rules) while leaving visual effects on the global generator. That is a large, invasive, well-scoped refactor best done as its own change — and it also unblocks §7's deterministic tests. Shipping `srand`-only now was rejected as a misleading half-measure.)*
- [ ] 🟡 Run modifiers / relic-style variety and unlock-gated content for long-tail replay. *(Not started 2026-07-14 — lowest priority in §2. Now cheap to add on top of the campaign: a data-driven per-night modifier deck (e.g. "Blood Moon: fares +20%, +1 rule", "Whiteout: visibility down, quota −10%") selected in `begin_night`, applied through the existing `minimum_earnings`/`SkillModifiers`/rule-count hooks, and surfaced on the briefing next to the new NIGHT N OF M label.)*
