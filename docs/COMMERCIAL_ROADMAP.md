...
- [ ] 🔴 Replace procedurally-assembled mid-ride events with an **authored event deck**. Today events are stitched from 4 hardcoded title/description templates in `ride_service.rs:662-830` with generic choices; `src/data/event.rs` is types-only and no event JSON exists. Target: dozens of hand-written, branching events in `assets/`.
- [ ] 🟠 Grow the passenger roster beyond 16 (target 30–40 for a roguelite), and **make passenger selection data-driven** — spawn weights by weather/time/season are hardcoded per passenger id 1–16 in `passenger_service.rs:132-269`, so adding a passenger requires code edits.
