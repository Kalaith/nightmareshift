...
- Rerun the balance sweep in `docs/balance_sweep.md` with the same seeds to re-measure after the meltdown-seam work landed: the six severed rule→guideline links are restored (pinned by test), the first meltdown now holds at the brink for one leg instead of killing, and the comfort skill branch gives cab actions a once-per-ride soothing floor. A 7-seed full-campaign probe saw zero meltdown deaths (failures were clock/hidden-rule), so the monoculture appears broken — the sweep's 45-shift grid is the instrument to confirm it and to price the comfort tier costs properly.
- Tune The Last Fare (night 6) into a real climax — it gates correctly (whole roster mastered) and Death spawns as the only fare, but the delivery itself measured gentle (one ride, high fuel). Candidates: max difficulty rule draw is already in force; add authored pressure on his ride (forced weather, no refuel, his rule always in force).
